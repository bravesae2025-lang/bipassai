...
The user approved **at most THB 10 total** for remaining targeted checks and a website check. Two live website calls have now used approximately **THB 0.939892** under the buffered estimate below. Do not resume the bulk matrix or interpret the user's separate THB 1,000 account budget as a testing allowance.
...
Both embedded and dedicated Railway consoles returned **WebSocket connection failed**. The user approved deploying and testing through the actual website instead. The standalone capped profile script remains unexecuted. The successful website requests establish that this project's provider accepted these calls; they do not verify any particular billing setting.

### Post-correction live result

`bf37a92` was pushed and Railway deployment `81753b0e` became Active. The same 73-word Custom/Balanced draft completed in 22,815 ms with two Structure groups, no repair and no mechanics call. Usage: 1,196 input, 393 output, 4,581 thinking tokens, approximately THB 0.511752 at the buffered rate. Combined follow-up totals: **two website submissions, 146 input words, two provider calls, 11,437 total provider tokens, approximately THB 0.94**. App pricing is 292 credits per submission (584 expected total); the first UI displayed 292 used, but a clean authoritative balance-delta verification was not captured. No other paid follow-up calls were made.

The retest used 76 words across three sentences, versus four before the prompt correction. “complete” became “finish” and “concentrate” became “focus”; the result now includes coordinated independent clauses joined with “but” and “and”. The live desktop screenshot showed wrapping Structure groups and readable controls without horizontal overflow. This is an individual improvement, not a corpus quality pass: subordinate clauses remain frequent, “consequently” became chronological “then”, and the purpose phrasing “to give students more time” became potentially outcome-asserting “so students had more time”. These are unresolved fidelity concerns, so this example is **not** an exact-meaning release approval. My Level's latest measured-profile priority was not retested live. No new mobile, extension upload, or live regeneration verification is claimed this round. Paid testing stopped after this comparison.
