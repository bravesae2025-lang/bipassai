...
if (!settingsHtml.includes('id="reset-onboarding-btn"')
    ...
    || !appJs.includes('const MY_LEVEL_GUIDE_HANDOFF_MS = 1500')
    ...
    || !appJs.includes("[pointer, hit].forEach(element => element.classList.add('is-cleared'))")
    || !appJs.includes("startMyLevelTour({ onClose, seamless: true })")
...
if (!styleCss.includes('.col-customize .profile-rim-feedback::after')
    ...
    || !styleCss.includes('.col-customize #my-style-inputs.profile-selection-surface::after')
    || !styleCss.includes('--profile-rim-exit: 680ms')
    || !styleCss.includes('--profile-rim-exit-ease: cubic-bezier(0.4,0,0.2,1)')
    || !styleCss.includes('--profile-rim-angle var(--profile-rim-exit) var(--profile-rim-exit-ease)')
    ...
    || !styleCss.includes('.profile-needs-complete .profile-selection-surface::after')) {
  add(styleFile, 'My Level must recess while off, highlight its inner profile surface instead of the whole section, dim presets, and show a red incomplete warning');
}
if (!appJs.includes('const profileSelected = myLevelSelectionPending || completedProfileActive')
    ...
    || !appJs.includes('syncProfileSelectionSurface(profileSelected)')
    || !appJs.includes('return [profileOption, getProfileSelectionSurface()].filter(Boolean)')
