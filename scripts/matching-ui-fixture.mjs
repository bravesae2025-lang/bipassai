...
app.get('/qa-frame', (req, res) => {
  ...
  const view = req.query.view === 'custom' ? '/home' : '/qa-long';
...
app.post('/api/adjust-level', (req, res) => {
  console.log('QA generation request', JSON.stringify({ structureMode: req.body.structureMode, level: req.body.level }));
  res.json({ cleanText: req.body.text, result: req.body.text, changes: [], structureMode: req.body.structureMode || 'keep', creditsUsed: 0, creditsRemaining: 100000 });
});
